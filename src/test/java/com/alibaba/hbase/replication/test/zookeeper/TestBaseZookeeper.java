...
/**
 * 类TestBaseZookeeper.java的实现描述：TODO 类实现描述 
 * @author zalot.zhaoh Apr 1, 2012 11:37:53 AM
 */
public class TestBaseZookeeper {

    protected static final int           zkClusterCount    = 4;
    protected static final int           hbaseClusterCount = 3;
    ...
    public static void createScopeTable(Configuration conf, int scope) throws Exception {
        HBaseAdmin admin = new HBaseAdmin(conf);
        for (int x = 1; x < scope + 1; x++) {
            HTableDescriptor htable = new HTableDescriptor(TABLE + x);
            HColumnDescriptor hcolumnA = new HColumnDescriptor(COLA);
            HColumnDescriptor hcolumnB = new HColumnDescriptor(COLB);
            htable.addFamily(hcolumnA);
            htable.addFamily(hcolumnB);
    ...
    public static void wirteScopeTable(Configuration conf, int scope, int size) throws Exception {
        HTablePool pool = new HTablePool(conf, scope);
        Random rnd = new Random();
        
        while (true) {
            int tbrnd = rnd.nextInt(scope);
            pool.getTable(TABLE + tbrnd);
        }
    }
