package com.alibaba.hbase.test.regionserver;
...
/**
 * HRegionServer 分析 <BR>
 * 类TestHRegionServerAnalyze.java的实现描述：TODO 类实现描述
 * 
 * @author zalot.zhaoh Mar 5, 2012 6:18:04 PM
 */
public class TestHRegionServerAnalyze {
    ...
    private static String[]                  TABLE_STRS = new String[] { TABLE_STR + "A", TABLE_STR + "B",
            TABLE_STR + "C", TABLE_STR + "D"           };
    ...
    public static Configuration getConf() throws Exception {
        ...
        if (TEST_UTIL.getZkCluster() == null) TEST_UTIL.startMiniZKCluster();
        ...
        TestHRegionServerAnalyze.conf = conf;
    ...
    public void testPut() throws Exception {
        for (String tableStr : TABLE_STRS) {
            ...
            for (int x = 0; x < 10000; x++) {
                ...
                put.add(COL, "name".getBytes(), getRndString(null).getBytes());
                puts.add(put);
            }
            table.put(puts);
            table.close();
    ...
    private Cpx printJvmInfoAndGetHRegionByTable(String table) throws IOException {
        ...
        for (JVMClusterUtil.RegionServerThread hregionServer : hbaseCluster.getLiveRegionServers()) {
            long count = 0;
            ...
            // }
            // }
            // System.out.println("    || region hlog [" + regionServer.getRpcServer().getListenerAddress() + "]" + file
            // + " files is [" + files.length + "] count = [" + currentSize + "]");
            // System.out.println("    ||----------------------------------------------------------------");
