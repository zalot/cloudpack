...
public class MyHLogTest {
	...
	// 该测可以单独建立， 直接运行即可
	// 可以观察 /root/hlog 和 /root/hlogold 目录的运行状态
	@Test
	public void testLocalHLogWrite() throws Exception {
		...
		HRegionInfo hriA = new HRegionInfo(TABLEA, HConstants.EMPTY_START_ROW,
				HConstants.EMPTY_END_ROW);
		HRegionInfo hriB = new HRegionInfo(TABLEB, HConstants.EMPTY_START_ROW,
				HConstants.EMPTY_END_ROW);
		...
		for (int i = 0; i < 10; i++) {
			cols.add(new KeyValue(row, Bytes.toBytes("column"), Bytes
					.toBytes(Integer.toString(i)), timestamp,
					new byte[] { (byte) (i + '0') }));
		}
		...
		for (int x = 0; x < 100; x++) {
			...
			if (x % 2 == 0) {
				long complateId = -1;
				try {
					complateId = hlog.startCacheFlush(hriA
				...
				if (complateId > 0) {
					hlog.completeCacheFlush(hriA.getEncodedNameAsBytes(),
							hriA.getTableName(), complateId, hriA.isMetaRegion());
					hlog.rollWriter();
				}
			} else{
				long complateId = -1;
				try {
					complateId = hlog.startCacheFlush(hriB
							.getEncodedNameAsBytes());
				} catch (Exception ec) {
				...
				if (complateId > 0) {
					hlog.completeCacheFlush(hriB.getEncodedNameAsBytes(),
							hriB.getTableName(), complateId, hriB.isMetaRegion());
					hlog.rollWriter();
				}
			}
		}
